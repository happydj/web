...
//添加水星
const Mercury = loadPlanet("mercury", 2, 20, 0.002, 0.002);
planets.push(Mercury);
//添加金星
const Venus = loadPlanet("venus", 4, 30, 0.012, 0.002);
planets.push(Venus);
//添加地球
const Earth = loadPlanet("earth", 5, 40, 0.001, 0.002);
planets.push(Earth);
//添加火星
...
function loadPlanet(name, radius, position, speed, rotationSpeed) {
  ...
  clickablePlanets.push(planetSystem)
  if (name === "saturn") {
...
function onCanvasClick(e) {
  ...
  if (intersects.length > 0) {
    ...
    //匹配跳转配置
    const planetConfig = planetNameMap[targetPlanet.name]

    if (planetConfig) {
      console.log('点击了${planetConfig.name},跳转到：${planetConfig.url');
      // 跳转页面（可替换为window.open 新窗口打开）
      window.location.href = "./" + planetConfig + ".html";
    }
  }
}
